...
function CloseIncident() {
  ...
  const handleAssignPDCSubmit: SubmitHandler<IImsCloseFormData> = (values) => {
    loader.show();
    closeIncident(values)
      .then(() => {
        alertToast.show("success", "Incident closed successfully", true, 2000);
        queryClient.invalidateQueries({
          predicate: (query) => query.queryKey[0] === "imsCloseQuery",
        });
